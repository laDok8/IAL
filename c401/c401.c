...
//prazdny
if(!(*RootPtr)){
     tBSTNodePtr item = malloc(sizeof(struct tBSTNode));
     if(!item)
         return;
     item->BSTNodeCont = Content;
     item->Key = K;
     item->LPtr = NULL;
     item->RPtr = NULL;
    (*RootPtr) = item;
...
**/
    tBSTNodePtr tempR = (*RootPtr)->RPtr;

    if(tempR){
        //vpravo
        ReplaceByRightmost(PtrReplaced,&tempR);
    }
    else{
        //nema podstrom nebo jen levy ( nejdu doleva)
...
if((*RootPtr)->Key == K){
    ...
    if(!(*RootPtr)->LPtr && !(*RootPtr)->RPtr){
        ...
        if((*RootPtr)->LPtr) {
            ...
            ReplaceByRightmost((*RootPtr),&(*RootPtr)->RPtr);
            BSTDelete(&(*RootPtr)->RPtr,(*RootPtr)->Key);
        }
    }
